...
$professionals = $agenda->list('profesionales');
...
const pros=<?= json_encode($professionals, JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_AMP|JSON_HEX_QUOT) ?>;
...
function openBlock(e){stop(e);openModal('Bloquear agenda',`<form class="space-y-3" onsubmit="saveBlock(event)"><label class="block text-sm">Motivo<input required name="motivo" placeholder="Reunión interna, vacaciones, descanso…" class="mt-1 border rounded-xl p-2.5 w-full"></label><label class="block text-sm">Inicio<input required type="datetime-local" name="inicio" value="${agendaDate}T12:00" class="mt-1 border rounded-xl p-2.5 w-full"></label><label class="block text-sm">Fin<input required type="datetime-local" name="fin" value="${agendaDate}T13:00" class="mt-1 border rounded-xl p-2.5 w-full"></label><label class="block text-sm">Profesional<select name="profesional_id" class="mt-1 border rounded-xl p-2.5 w-full"><option value="">Toda la agenda</option>${pros.filter(p=>Number(p.activo)).map(p=>`<option value="${p.id}">${esc(p.nombre)}</option>`).join('')}</select></label><button class="w-full bg-slate-900 text-white rounded-xl p-3 font-semibold">Bloquear horario</button></form>`)}
