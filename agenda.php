...
<div class="max-w-7xl mx-auto space-y-6">
  ...
  <section class="bg-white border border-slate-200 rounded-3xl p-5 md:p-6 shadow-sm">
    ...
    <div class="rounded-2xl border border-slate-100 p-4 mt-4"><div class="flex justify-between items-center mb-3"><div><h3 class="font-semibold text-sm">Horarios disponibles</h3><p class="text-xs text-slate-400">Cada horario pertenece a un servicio de una agenda concreta.</p></div><button type="button" onclick="openHours(event)" class="text-emerald-700 text-xs font-semibold">+ Añadir horario</button></div><?php foreach($hours as $item): ?><div class="flex justify-between gap-2 py-2 border-t border-slate-100"><div><b class="text-sm"><?= htmlspecialchars($item['servicio']??'Servicio') ?></b><p class="text-xs text-slate-400"><?= htmlspecialchars(($item['negocio']??'').' · '.($item['sucursal']??'').' · '.($item['agenda']??'')) ?> · <?= ['Dom','Lun','Mar','Mi','Jue','Vie','Sáb'][(int)$item['dia_semana']] ?> <?= htmlspecialchars(substr($item['hora_inicio'],0,5).'–'.substr($item['hora_fin'],0,5)) ?></p></div><div class="abm-actions shrink-0"><button title="Editar" onclick="editHours(<?= htmlspecialchars(json_encode($item),ENT_QUOTES,'UTF-8') ?>)">✏️</button><button title="<?= !empty($item['activo'])?'Desactivar':'Activar' ?>" onclick="toggleEntity('horarios',<?= (int)$item['id'] ?>,<?= !empty($item['activo'])?'0':'1' ?>)"><?= !empty($item['activo'])?'⏻':'◯' ?></button><button title="Eliminar" onclick="deleteEntity('horarios',<?= (int)$item['id'] ?>)">🗑️</button></div></div><?php endforeach; ?><?php if(!$hours): ?><p class="text-xs text-slate-400">Todavía no hay horarios configurados.</p><?php endif; ?></div>
  </section>
  <section class="bg-white border border-slate-200 rounded-3xl p-5 md:p-6 shadow-sm">
