...
<div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/45" role="dialog" aria-modal="true" aria-labelledby="prospect-modal-title">
  <div class="w-full max-w-3xl max-h-[92vh] overflow-hidden rounded-2xl bg-white shadow-2xl">
    <form method="POST" class="flex max-h-[92vh] flex-col"><input type="hidden" name="prospecto_id" value="<?= $seleccionado['id'] ?>">
      <div class="flex items-center justify-between border-b border-slate-200 px-6 py-4"><div><p class="text-xs font-semibold text-emerald-600">Ficha comercial</p><h2 id="prospect-modal-title" class="mt-1 font-bold text-slate-800">Editar prospecto</h2></div><div class="flex items-center gap-4"><span class="text-2xl font-bold text-slate-800"><?= (int)$seleccionado['puntaje'] ?><small class="text-xs text-slate-400">/100</small></span><a href="prospectos.php?search=<?= urlencode($search) ?>&heat=<?= urlencode($heat) ?>" class="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700" aria-label="Cerrar ficha">✕</a></div></div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-3"><label class="text-xs font-medium text-slate-600">Interés<select name="nivel_interes" class="mt-1 w-full rounded-lg border border-slate-200 px-2 py-2 text-sm"><?php foreach(['bajo','medio','alto'] as $v): ?><option <?= $seleccionado['nivel_interes']===$v?'selected':'' ?>><?= $v ?></option><?php endforeach; ?></select></label><label class="text-xs font-medium text-slate-600 sm:col-span-2">Temperatura<select name="temperatura" class="mt-1 w-full rounded-lg border border-slate-200 px-2 py-2 text-sm"><?php foreach(['frio','tibio','caliente','muy_caliente'] as $v): ?><option value="<?= $v ?>" <?= $seleccionado['temperatura']===$v?'selected':'' ?>><?= str_replace('_',' ',$v) ?></option><?php endforeach; ?></select></label></div>
