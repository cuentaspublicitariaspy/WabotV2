...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif ($action === 'change_password') { if($esSuperAdmin){ $id=(int)($_POST['user_id']??0); $pw=$_POST['new_password']??''; if($id>0&&$pw!==''){ $db->prepare("UPDATE usuarios SET password_hash=? WHERE id=?")->execute([password_hash($pw,PASSWORD_BCRYPT),$id]); $message='Contraseña cambiada';$messageType='success';}} }
    elseif ($action === 'change_role') { if($esSuperAdmin){ $id=(int)($_POST['user_id']??0); $rol=$_POST['rol']??''; if($id>0&&in_array($rol,['super_admin','admin','agent'])){ $db->prepare("UPDATE usuarios SET rol=? WHERE id=?")->execute([$rol,$id]); $message='Rol cambiado a '.$rol;$messageType='success';}} }
    ...
    <table class="w-full text-xs">
      ...
      <tbody class="divide-y divide-slate-50">
        ...
        <tr class="hover:bg-slate-50">
          ...
          <td class="px-3 py-2">
            <?php if ($esSuperAdmin && $u['id'] !== $user['id']): ?>
            <form method="POST" class="flex items-center gap-1">
              <input type="hidden" name="action" value="change_role">
              <input type="hidden" name="user_id" value="<?= $u['id'] ?>">
              <select name="rol" onchange="this.form.submit()" class="text-[11px] border border-slate-200 rounded-lg px-1.5 py-0.5 outline-none focus:ring-2 focus:ring-emerald-500 bg-white">
                <option value="agent" <?= $u['rol']==='agent'?'selected':'' ?>>agent</option>
                <option value="admin" <?= $u['rol']==='admin'?'selected':'' ?>>admin</option>
                <option value="super_admin" <?= $u['rol']==='super_admin'?'selected':'' ?>>super_admin</option>
              </select>
            </form>
            <?php else: ?>
          ...
          <td class="px-3 py-2 whitespace-nowrap">
            ...
            <?php if ($esSuperAdmin): ?>
            <button onclick="var p=prompt('Nueva contraseña para <?= htmlspecialchars($u['nombre'], ENT_QUOTES) ?>:');if(p&&p.length>=4){var f=document.createElement('form');f.method='POST';f.innerHTML='<input name=action value=change_password><input name=user_id value=<?= $u['id'] ?>><input name=new_password value='+encodeURIComponent(p)+'>';document.body.appendChild(f);f.submit()}" class="w-7 h-7 rounded-lg bg-sky-50 text-sky-500 hover:bg-sky-100 hover:text-sky-600 transition inline-flex items-center justify-center" title="Cambiar contraseña"><i class="bi bi-key text-sm"></i></button>
            <?php endif; ?>
            <?php if ($u['rol'] !== 'admin'): ?>
