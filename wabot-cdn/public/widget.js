(function () {
  ...
  function messageId() { return Date.now().toString(36) + '-' + Math.random().toString(36).slice(2, 10); }
  function persist(role, content, requestedId) {
    var id = requestedId || messageId();
    var items = outbox();
  ...
  function submit(text) { text = (text || '').trim(); if (!text || !historyLoaded) return; input.value = ''; send.disabled = true; var visitorMessageId = messageId(), assistantMessageId = messageId(); var all = messages(); all.push({ role: 'visitor', content: text, time: now() }); save(all); render(all); persist('visitor', text, visitorMessageId); typing.classList.add('show'); box.scrollTop = box.scrollHeight; var history = all.slice(0, -1).map(function (item) { return { role: item.role === 'visitor' ? 'user' : 'assistant', content: item.content }; }); var request = new XMLHttpRequest(); request.open('POST', endpoint('send'), true); request.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded'); request.onreadystatechange = function () { if (request.readyState !== 4) return; typing.classList.remove('show'); send.disabled = false; if (request.status === 200) { try { var response = JSON.parse(request.responseText); if (response.success && response.message && response.message.content) { var updated = messages(); updated.push({ role: 'assistant', content: response.message.content, time: now() }); save(updated); render(updated); persist('assistant', response.message.content, assistantMessageId); } } catch (e) {} } }; request.send('key=' + encodeURIComponent(apiKey) + '&message=' + encodeURIComponent(text) + '&history=' + encodeURIComponent(JSON.stringify(history)) + '&session_id=' + encodeURIComponent(sessionId) + '&visitor_message_id=' + encodeURIComponent(visitorMessageId) + '&assistant_message_id=' + encodeURIComponent(assistantMessageId)); }
