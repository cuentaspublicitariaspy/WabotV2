(function () {
  ...
  var html = '' +
    '<style>' +
    ':host{all:initial;display:block!important;position:fixed!important;inset:0!important;width:100vw!important;height:100vh!important;overflow:visible!important;pointer-events:none!important;z-index:2147483647!important;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;isolation:isolate!important}' +
    '*,*:before,*:after{box-sizing:border-box;font-family:inherit}' +
    '.wc-shell{position:absolute;right:24px;bottom:24px;z-index:2147483646;display:flex;flex-direction:column;align-items:flex-end;pointer-events:none}' +
    ...
    '.wc-suggestions,.wc-inputbar,.wc-footer{position:relative!important;z-index:2!important;background:#fff!important}' +
    '</style>' +
  ...
  function fetchConfig() { var request = new XMLHttpRequest(); request.open('GET', endpoint('config') + '?key=' + encodeURIComponent(apiKey) + '&origin=' + encodeURIComponent(window.location.origin), true); request.onreadystatechange = function () { if (request.readyState !== 4) return; if (request.status !== 200) { unlockChat(); return; } try { var data = JSON.parse(request.responseText); if (!data.success || !data.config) { unlockChat(); return; } var config = data.config; if (data.storage_base) { storeUrl = storeUrl || data.storage_base + '/api/widget/store.php'; historyUrl = historyUrl || data.storage_base + '/api/widget/messages.php'; } if (config.primary_color) { color = config.primary_color; shell.style.setProperty('--wc-color', color); } if (config.welcome_title) root.querySelector('#wc-title').childNodes[0].nodeValue = config.welcome_title; if (config.welcome_subtitle) root.querySelector('#wc-sub').textContent = config.welcome_subtitle; flushOutbox(); if (historyLoaded) unlockChat(); else if (historyUrl) loadHistory(); else unlockChat(); } catch (e) { unlockChat(); } }; request.send(); }
