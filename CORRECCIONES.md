...
- Regla general inviolable: ninguna capacidad, habilitada o deshabilitada, puede hacer que el Bot pierda su comprensión conversacional. La restricción solo bloquea la acción concreta de esa capacidad; una intención nueva, una corrección, un mensaje para el equipo, datos compartidos voluntariamente o un cambio de tema deben comprenderse y atenderse normalmente.
- Toda respuesta del Chatbot debe quedar en Conversaciones de WC. El guardado es redundante e idempotente: WS lo reenvía transitoriamente a WC y el navegador conserva un outbox de reintento con los mismos IDs, sin almacenar mensajes en WS ni duplicarlos en WC.
- Guardar el chat y el mensaje es siempre la operación prioritaria. La extracción o actualización de prospectos, las migraciones de compatibilidad y cualquier enriquecimiento con IA son secundarios: si fallan, jamás pueden impedir que la conversación aparezca en WC.
- Si se reutiliza la misma ventana del Chatbot y la persona declara un nombre, teléfono o correo nuevo, WC debe refrescar el contacto visible con el último dato realmente declarado; nunca debe conservar el nombre anterior solo porque la sesión del navegador ya estaba identificada. Los teléfonos se normalizan antes de actualizar la ficha.
