...
$cacheBuster = '?t=' . time();
$avatarUrl = $userFoto ? $userFoto . $cacheBuster : "https://ui-avatars.com/api/?name=" . urlencode($user['nombre']) . "&background=10b981&color=fff";
$navLinks = [
...
$adminNavLinks = [
    ...
    'config_api' => ['label' => 'Conexión WhatsApp', 'icon' => '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.066 2.573c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.573 1.066c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.066-2.573c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>', 'href' => 'configuracion_api.php'],
        ...
        <aside class="w-64 bg-slate-900 text-slate-300 flex flex-col justify-between py-6 shrink-0 shadow-xl">
            ...
            <nav class="flex-1 px-4 space-y-8">
                ...
                <div class="space-y-1">
                    ...
                    <?php if ($key === 'conocimiento' && $user['rol'] !== 'admin') continue; ?>
                    <a href="<?= $link['href'] ?>" class="flex items-center gap-3 px-4 py-3 rounded-lg transition-all <?= $activePage === $key ? 'bg-emerald-600 text-white font-medium shadow-lg shadow-emerald-900/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white' ?>">
                        <?= $link['icon'] ?>
                        <span><?= $link['label'] ?></span>
                    </a>
        ...
        document.getElementById('profile-form').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    var fotoUrl = data.foto ? data.foto + '?t=' + Date.now() : '';
                    if (fotoUrl) {
